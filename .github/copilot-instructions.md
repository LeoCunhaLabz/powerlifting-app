...
- `apps/landing/` — landing page pública (Astro 5 + ilhas React, CSS puro) servida na raiz de `onyxtreino.com.br` pelo mesmo nginx do app. Reutiliza os cálculos do app pelos aliases `@onyx/calc` → `apps/web/src/utils/powerlifting.ts` e `@onyx/strength` → `apps/web/src/utils/strength.ts`. Sem bibliotecas de efeito (reactbits/gsap/motion saíram na #315): superfícies chapadas, display em Archivo, brass contido; o único movimento é o `useCountUp`, atrás de `useMotionAllowed()`. CSP própria em `nginx-landing-security-headers.conf`. Detalhes em [AGENTS.md](../AGENTS.md#landing-page-pública--appslanding-issue-250).
- `packages/shared/` — tipos de domínio compartilhados, pacote `@powerlifting/shared`.
- Comandos raiz: `npm run dev/build/lint/test` (web), `npm run dev:api/build:api/lint:api/test:api/start:api` (api), `npm run dev:landing/build:landing/check:landing/lint:landing/test:landing` (landing), `npm run test:e2e` (Playwright, golden path + handoff da calculadora — requer Postgres em `E2E_DATABASE_URL`), `npm run test:e2e:landing` (smoke Playwright da landing, só estático).
