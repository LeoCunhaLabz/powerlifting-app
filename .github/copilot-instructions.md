...
- Comandos raiz: `npm run dev/build/lint/test` (web), `npm run dev:api/build:api/lint:api/test:api/start:api` (api), `npm run dev:landing/build:landing/check:landing/lint:landing/test:landing` (landing), `npm run test:e2e` (Playwright, golden path — requer Postgres em `E2E_DATABASE_URL`), `npm run test:e2e:landing` (smoke Playwright da landing, só estático).
