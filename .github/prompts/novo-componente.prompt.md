...
Crie o componente `${input:nomeComponente:Nome do componente (ex.: ProgressRing)}` em `src/components/`.

Diretrizes (veja [AGENTS.md](../../AGENTS.md)):

1. `React.FC` com `interface` de props tipada. Textos visíveis em **pt-BR**.
2. **Estilo:** use as CSS variables de [src/index.css](../../src/index.css) (cores, `--radius-*`, `--transition-*`). Não hardcode hex que já tenha token. Sem Tailwind/CSS-in-JS libs.
...
Padrões de referência: [src/components/PlateVisualizer.tsx](../../src/components/PlateVisualizer.tsx) e [src/components/RestTimer.tsx](../../src/components/RestTimer.tsx).
